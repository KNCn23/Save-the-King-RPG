...
def guard_encounter():
    ...
    while mistakes < 4: #If it is less than 3, it does not grant the third right.
        user_answer = input("\n Your Answer:").strip().lower()
        
        if user_answer in selected_question["valid_answers"]:
            slow_print("Guard: ... Correct. You are wiser than you look.", style="dialogue")
            return True
        else:
            mistakes += 1
            slow_print(f"Guard: WRONG! You have {3-mistakes} chances left.", style="dialogue")
...
def villager_talk(my_hero, world):
    ...
    # 1. DURUM: BANDIT LEADER ÖLDÜYSE (KAHRAMANSIN)
    if world.get("bandit camp", {}).get("cleared", False) == True:
        ...
        # random.choice listeden rastgele birini seçer
        slow_print(f"[bold green]{random.choice(hero_dialogues)}[/bold green]")
        
    # 2. DURUM: BANDIT LEADER HALA YAŞIYOR (KAOS VAR)
    else:
        chance = random.randint(1, 100)
        
        
        # --- %10 İHTİMALLE SAVAŞ (Angry Villager) ---
        if chance <= 10:
            slow_print("[bold red]VILLAGER: I won't let you hurt anyone else! DIE TRAITOR![/bold red]")
        ...
        # --- %30 İHTİMALLE SUÇLAYICI / KORKMUŞ ---
        if chance <= 40:
            ...
            slow_print(f"[bold red]{random.choice(suspicious_dialogues)}[/bold red]")
            slow_print("(The villager backs away slowly, holding a kitchen knife.)")

        # --- %30 İHTİMALLE BİLGİ VERİCİ ---
        elif chance <= 70:
            info_dialogues = [
            ...
            slow_print(f"[italic cyan]{random.choice(info_dialogues)}[/italic cyan]")

        # --- %30 İHTİMALLE DOSTANE / UMUTSUZ ---
        else:
            friendly_dialogues = [
            ...
            slow_print(f"{random.choice(friendly_dialogues)}")
...
def check_locations(my_hero, world, currentlocation, inventory):
    
    # 1. TÜCCAR KONTROLÜ
    if currentlocation == "town square":
        merchant_event(my_hero, inventory)

    # 2. KÖYLÜ KONTROLÜ (Giriş katları)
    ...
    # 4. GUARDS
        ...
        if quiz_results == True:
            slow_print("\nGuard Commander: You possess great wisdom. The gates are open for you!")
            slow_print("You can enter the Castle safely!, Go north!!")
        else:
            slow_print("\nGuard Comander: You are a fool and spy! EXECUTE HIM!")
            slow_print("The guards seize you. The last thing you see is the executioner's axe...")
            my_hero.isAlive = False
    
    # 5. RASTGELE SAVAŞ (Belli odalar)
    ...
    # 6. ARCHIVES
        ...
        if "chest_opened" not in world["archives"]:
            # Puzzle fonksiyonuna hero ve world gönderiyoruz
            puzzle_result = archives_puzzle(my_hero, world)
            
            if puzzle_result == True:
                slow_print("Inside the chest, you find a [loot]Masterwork Armor[/loot]!")
                slow_print("You equipped the armor. It gives you more attack power!")
                
                
                my_hero.AttackPower += 15 
                
                world["archives"]["chest_opened"] = True 
            else:
                slow_print("You kick the chest in frustration, but it won't budge.")
                world["archives"]["chest_opened"] = False
        else:
            slow_print("The chest in the corner is empty and open.")
                
    # 7. BANDIT CAMP BOSS
    elif currentlocation == "bandit camp":
        # Eğer boss zaten öldüyse tekrar çıkmasın
        if world["bandit camp"].get("cleared", False) == False:
            ...
            if not fight_result:
                ...
                slow_print("\n*** VICTORY ***")
                slow_print("You have defeated the Bandit Leader! The remaning bandits flee in terror.")
                slow_print("You found the Leader's [loot]Heavy Armor[/loot] and put it on!")
                my_hero.armor(50)
                # Boss öldü diye işaretliyoruz
